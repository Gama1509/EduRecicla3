...
export default function ProductViewPage() {
    ...
    const allowedStatusOptions = [
        TransactionStatus.INTEREST_CANCELLED,
        TransactionStatus.BUYER_CANCELLED_SOLD_OUT_PARTIAL,
        TransactionStatus.BUYER_CANCELLED_SOLD_OUT_TOTAL
    ];

    const getStatusGlow = (status: TransactionStatus) => {
        switch (status) {
            case TransactionStatus.INTEREST_CANCELLED:
            case TransactionStatus.BUYER_CANCELLED_SOLD_OUT_PARTIAL:
            case TransactionStatus.BUYER_CANCELLED_SOLD_OUT_TOTAL:
                return 'red';
    ...
    useEffect(() => {
        const fetchTransactions = async () => {
            try {
                const res = await api.get<TransactionsProfileDto[]>(`/transactions/getInterestCancelledTransactionsByBuyer`);
        ...
        <div className="p-6 w-full overflow-x-auto">
            <div className="flex flex-col items-center mb-8 gap-6">
                <h1 className="text-3xl font-bold text-text-primary-light dark:text-text-primary-dark text-center">
                    Productos en los que cancele mi interés
                </h1>
                {loading && (
